...
@if (auth()->check() && auth()->user()->isAdmin())
    <x-app-layout>
        ...
        <div class="container mx-auto">
            ...
            @if (auth()->check() && auth()->user()->isAdmin())
                <div class="mb-4">
                    <a href="{{ route('additem') }}" class="bg-green-500 text-white px-4 py-2 rounded hover:bg-red-600">Add
                        Item</a>
                </div>
            @endif
            <!-- Search Form -->
                    ...
                    <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg">
                        <div class="p-6 text-gray-900 dark:text-gray-100">
                            ...
                            <button class="bg-yellow-500 text-white px-4 py-2 rounded hover:bg-blue-600 mb-2 details"
                                data-item-id="{{ $item }}" data-url="{{ route('cart-items.store') }}">Details</button>
                            @if ($item->status == 0)
