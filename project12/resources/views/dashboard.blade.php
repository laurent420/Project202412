                    ...
                    @foreach ($items as $item)
                        <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg">
                            <div class="p-6 text-gray-900 dark:text-gray-100">
                                ...
                                <p class="text-sm mb-2">Amount Left: {{ $item->quantity }}</p>
                                <img src="{{ asset($item->picture) }}" alt="{{ $item->name }}" class="w-full mb-2">
                                <button class="bg-yellow-500 text-white px-4 py-2 rounded hover:bg-blue-600 mb-2 details" data-item-id="{{ $item }}" data-url="{{ route('cart-items.store') }}">Details</button>
